...
/// The inputs the relay settled for a frame, every slot's in slot order, each a flags byte followed by
/// `inputSize` bytes of input.
struct Confirmed
{
...
/// The answer to a ping: the stamp it carried and the frame the relay has confirmed so far.
struct Pong
{
