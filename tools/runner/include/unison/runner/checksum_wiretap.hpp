...
/// Stands in front of the relay: it hands every message and every departure on and, on the way, writes the
/// checksums the clients report into a ledger under the client that sent them, the clients numbered in the
/// order of their peers. Messages from anyone else, and those it cannot read, go on untouched.
